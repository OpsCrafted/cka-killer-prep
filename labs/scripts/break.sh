...
if [ -z "$LAB" ]; then
  ...
  echo "  lab-11-etcd-corruption       Restore etcd from corruption"
  echo "  lab-12-kubelet-misconfigured Node misconfigured"
  echo "  lab-13-cluster-upgrade       Upgrade cluster nodes"
  echo "  lab-14-node-join             Join new nodes"
  echo "  lab-15-runtime-switch        Switch container runtime"
  echo "  lab-16-cni-migration         Migrate CNI plugin"
  echo "  lab-17-custom-api            Setup CRD"
  echo "  lab-18-webhook-config        Configure webhook"
  echo "  lab-19-audit-logging         Enable audit logging"
  echo "  lab-20-rbac-design           Design RBAC roles"
  echo "  lab-21-serviceaccount-binding Bind ServiceAccount"
  echo "  lab-22-backup-strategy       Backup and restore"
  echo "  lab-23-ingress-tls           Setup TLS ingress"
  echo "  lab-24-gateway-api           Gateway API"
  echo "  lab-25-netpol-namespace      Namespace isolation"
  echo "  lab-26-loadbalancer-expose   Expose LoadBalancer"
  echo "  lab-27-dns-debugging         Debug DNS issues"
  echo "  lab-28-service-discovery     Fix service discovery"
  echo "  lab-29-gatewayclass-binding  Bind GatewayClass"
  echo "  lab-30-multi-tenancy         Multi-tenancy ingress"
  echo "  lab-31-deployment-rolling    Rolling updates"
  echo "  lab-32-statefulset-storage   StatefulSet storage"
  echo "  lab-33-resource-limits       Enforce resource limits"
  echo "  lab-34-affinity-taints       Node affinity and taints"
  echo "  lab-35-priority-preemption   Priority and preemption"
  echo "  lab-36-hpa-scaling           Horizontal Pod Autoscaler"
  echo "  lab-37-pv-pvc-binding        Bind PV and PVC"
  echo "  lab-38-storageclass-default  Default StorageClass"
  echo "  lab-39-local-storage         Local storage provisioning"
  echo "  lab-40-pvc-expansion         Expand PVC"
  exit 0
fi
